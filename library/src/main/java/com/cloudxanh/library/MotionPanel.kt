...
open class MotionPanel @JvmOverloads constructor(
    context: Context, attrs: AttributeSet? = null, defStyleAttr: Int = 0
) : RelativeLayout(context, attrs, defStyleAttr) {

    companion object {

        lateinit var binding: LayoutDraggablePanelBinding
        var DEBUG = true
    ...
    var needExpand = true// cần expand binding.appbarLayout
    ...
    var velocityTracker: VelocityTracker? =
        null // tốc độ khi  MotionEvent.ACTION_UP, MotionEvent.ACTION_CANCEL
    ...
    init {
        ...
        if (attrs != null) {
            val typedArray = context.obtainStyledAttributes(attrs, R.styleable.MotionPanel)
            mTempHeight = typedArray.getDimensionPixelSize(
                R.styleable.MotionPanel_height_when_max,
                200.toPx()
            )

            mPercentWhenMiddle =
                typedArray.getFloat(R.styleable.MotionPanel_percent_when_middle, 0.9f)

            mHeightWhenMin =
                typedArray.getDimensionPixelSize(R.styleable.MotionPanel_height_when_min, 80.toPx())

            mMarginEdgeWhenMin = typedArray.getDimensionPixelSize(
                R.styleable.MotionPanel_margin_edge_when_min,
                8.toPx()
            )

            mMarginBottomWhenMin = typedArray.getDimensionPixelSize(
                R.styleable.MotionPanel_margin_bottom_when_min,
                8.toPx()
            )

            mTempState = State.values()[typedArray.getInt(R.styleable.MotionPanel_state, 3)]
        ...
        binding.frameDrag.onTouchListener = object : DragLayout.OnTouchListener {
            ...
            override fun onInterceptTouchEvent(ev: MotionEvent?): Boolean {
                when (ev!!.action) {
                    MotionEvent.ACTION_DOWN -> {
                        firstViewDown = isViewUnder(binding.frameFirst, ev)
                        downY = ev.rawY
                    }
            ...
            private fun checkFrameFirstMove(ev: MotionEvent) {
                ...
                if (calculateDiff > scaledTouchSlop && firstViewDown) {
                    deltaY =
                        ev.rawY.toInt() - (binding.frameDrag.layoutParams as LayoutParams).topMargin
                    firstViewMove = true
                }
            }
    ...
    open fun initFrame() {
        ...
        if (mCurrentState == null) {
            val params = binding.frameFirst.layoutParams as CoordinatorLayout.LayoutParams
            params.behavior = DragBehavior(binding.frameSecond)
            binding.frameFirst.layoutParams = params
        }
        ...
        mHeightWhenMiddle =
            (height - mPercentWhenMiddle * mMarginBottomWhenMin - mPercentWhenMiddle * mMarginTopWhenMin).toInt()
        if (mCurrentState == null) {
    ...
    open fun getFrameDrag(): ViewGroup {
        return binding.frameDrag
    }

    open fun getFrameFirst(): ViewGroup {
        return binding.frameFirst
    }

    open fun getFrameSecond(): ViewGroup {
        return binding.frameSecond
    }
    ...
    open fun maximize() {
        ...
        when (mCurrentState) {
            State.MAX -> {
                binding.appbarLayout.resizeAnimation(-1, mTempHeight, 300) {
                    mHeightWhenMax = mTempHeight
    ...
    open fun refresh() {
        ...
        val layoutParams = binding.frameDrag.layoutParams as LayoutParams
        ...
        binding.frameDrag.layoutParams = layoutParams
        ...
        binding.toolbar.reHeight(toolBarHeight)
    ...
    open fun refreshFrameFirst() {
        ...
        binding.appbarLayout.reHeight(frameFistHeight.toInt())
    }
    ...
    private fun springYAnim(endValue: Float, onEnd: () -> Unit) {
        velocityY.springAnimation(
            0.toFloat(),
            mMarginTopWhenMin.toFloat(),
            mCurrentMarginTop.toFloat(),
            endValue,
            { value: Float ->
